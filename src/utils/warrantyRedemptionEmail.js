...
const reshapeArabic = arabicReshaper.default
  ? arabicReshaper.default.reshape
...
//   if (!text) return '';
//   try {
//     const reshaped = reshapeArabic(String(text));
//     return bidi.getVisualString(reshaped);
//   } catch (e) {
//     // console.warn('⚠️ Arabic reshape failed:', e.message);
//     return text;
//   }
// }
// Simple Arabic word reverser for PDFKit (fixes reversed order issue)
function toArabic(text = '') {
  if (!text) return '';
  // Split by spaces, reverse words, then join again
  // This makes e.g. "معرّف مندوب المبيعات" appear in correct order
  try {
...
/**
 * Bilingual Arabic-English PDF — English (right) | Value (center) | Arabic (left)
 * Uses prettyDate() for all date formatting
 */
function buildRedemptionPdf(data = {}) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      console.warn('⚠️ Cairo-Regular.ttf missing:', err.message);
      doc.font('Helvetica');
    }
    ...
    try {
      ...
      doc.image(logoPath, 230, 20, { width: 130 });
      doc.moveDown(3);
    } catch (e) {
    ...
    doc.fontSize(18).fillColor('black')
      .text( 'Loyalty Reward Receipt | ' + toArabic('سند  استلام مكافأة ولاء'), { align: 'center' });
    doc.moveDown(0.25);
    doc.fontSize(10).fillColor('#555')
      .text(`${prettyDate(new Date())} : ${toArabic('تاريخ ')} `, { align: 'right' });
    doc.moveDown(1);
    ...
    // ---- Table (English | Value | Arabic) ----
    const rows = [
      ['Name', `${data.name ?? 'Not Found'} `, 'الاسم'],
    //  ['Name', `${data.name ?? 'Not Found'} ${data.role ? `(${data.role})` : 'Not Found'}`, 'الاسم'],
      ['Customer Type', ` ${data.role ? `${data.role}` : 'Not Found'}`, 'نوع  العميل'],
      ['Phone', data.phone ?? 'Not Found', ' رقم  الجوال'],
      ['Customer ID (BSG)', data.bsgCustId ?? 'Not Found', 'معرّف  العميل  (BSG)'],
      ['Sales Rep ID', data.salesRepId ?? 'Not Found', 'معرّف  مندوب  المبيعات'],
      ['Reward', data.rewards ?? data.rewardId ?? 'Not Found', 'المكافأة'],
      ['Required Points', data.requiredPoints ?? data.pointsConsumed ?? 'Not Found', 'النقاط المطلوبة'],
      ['Location', data.location ?? 'Not Found', 'الموقع'],
      ['Requested Date', prettyDate(data.requestDate), 'تاريخ  الطلب'],
      ['Approval Date', prettyDate(new Date()), 'تاريخ  الموافقة'],
    ];

    const tableStartX = 60;
    const totalWidth = 480;
    const colEnglish = 160;
    const colValue = 160;
    const colArabic = 160;
    let y = doc.y;

    doc.font('Cairo').fontSize(12).fillColor('black');

    rows.forEach(([english, value, arabic]) => {
      if (!value || !String(value).trim()) return;

  const isLocation = english.toLowerCase().includes('location');
//      const isLocationOrName = ['location', 'name'].some(key =>
//   english.toLowerCase().includes(key)
// );


      // English label (Right)
      doc.font('Cairo').fontSize(12).text(english, tableStartX, y, {
        width: colEnglish,
        align: 'left',
      });

      // Value (Center)
      doc.text(value, tableStartX + colEnglish + 10, y, {
        width: colValue,
        align: 'center',
      });

      // Arabic label (Left)
      doc.text(toArabic(arabic), tableStartX + colEnglish + colValue + 20, y, {
        width: colArabic,
        align: 'right',
      });

      // Divider
     y = doc.y + (isLocation ? 20 : 6); // 🟢 reserve two lines for long location
//  y = doc.y + (isLocationOrName ? 20 : 6);

      doc.moveTo(tableStartX, y).lineTo(tableStartX + totalWidth, y)
    ...
    // ---- Signature Section ----
// doc.moveDown(1);
// doc.font('Cairo').fontSize(10).fillColor('black')
//   .text('Signature | ' + toArabic(' التوقيع'), { align: 'center' });
// doc.moveDown(0.8);
// ---- Signature Section ----
const pageWidth = doc.page.width - 84;
const colWidth = (pageWidth - 30) / 3;
const ySigStart = doc.y + 40; // starting point for the titles ABOVE boxes
const x1 = 42;
const x2 = x1 + colWidth + 15;
const x3 = x2 + colWidth + 15;

const drawSigBox = (x, titleEn, titleAr, name, customY = ySigStart) => {
  // Combine Arabic + English WITHOUT actual newline
  // (we'll render them as two separate text calls for perfect spacing)
  const titleArText = toArabic(titleAr);
  const titleEnText = titleEn;
  ...
  doc.font('Cairo')
    ...
    .fillColor('black')
    .text(titleArText, x, customY, {
      align: 'center',
      width: colWidth,
    });

  // --- Draw English title just below Arabic manually ---
  const afterArabicY = doc.y + lineGap;
  doc.text(titleEnText, x, afterArabicY, {
    align: 'center',
    width: colWidth,
  });

  // --- Now calculate total title height ---
  const titleHeight = (doc.y - customY) + 4; // +4 for consistent breathing space

  // --- Compute where box should start (always below text) ---
  const boxTop = customY + titleHeight + 6;

  // --- Draw the signature box ---
  doc.rect(x, boxTop, colWidth, 60)
    .strokeColor('#999')
    .stroke();

  // --- Label under box ---
  const labelY = boxTop + 65;
  doc.fontSize(10)
    .fillColor('#444')
    .text(toArabic('الاسم | Name'), x, labelY, {
      align: 'center',
      width: colWidth,
    });
  ...
  if (name) {
    doc.text(name, x, labelY + 15, { align: 'center', width: colWidth });
  }
};


const baseY = doc.y;
drawSigBox(x1, 'Sales Rep Signature', 'توقيع  مندوب المبيعات', '', baseY);
drawSigBox(x2, 'Branch Manager Signature', 'توقيع  مدير الفرع', '', baseY);
drawSigBox(x3, 'Customer Signature', 'توقيع  العميل', data.name ?? '', baseY);
    ...
    //   .text(toArabic('— برنامج ولاء مجموعة بن شيهون' ), { align: 'center' });
    // doc.fontSize(10).fillColor('#666')
    //   .text('Bin-Shihon Rewards Program', { align: 'center' });
// ---- Footer (bottom center, same line bilingual) ----
const footerY = doc.page.height - doc.page.margins.bottom - 20; // ~20pt above bottom edge
doc.moveTo(42, footerY - 10).lineTo(553, footerY - 10).strokeColor('#cccccc').stroke();

doc.fontSize(10).fillColor('#666')
  .text(  'Bin-Shihon Rewards Program |'+   toArabic('برنامج ولاء مجموعة بن شيهون  ') , 
...
//     doc.on('end', () => resolve(Buffer.concat(chunks)));
//     doc.on('error', reject);
//   // ✅ Arabic-capable font (same folder)
//     try {
//       const fontPath = path.join(__dirname, 'Cairo-Regular.ttf');
//       doc.registerFont('Cairo', fontPath);
//       doc.font('Cairo');
//     } catch (err) {
//       console.warn('⚠️ Failed to load Cairo-Regular.ttf:', err.message);
//       doc.font('Helvetica');
//     }
...
//     // --- Insert Logo ---
//     try {
//       const logoPath = path.join(__dirname, '../../uploads/logo.png'); // adjust if needed
//       doc.image(logoPath, 230, 20, { width: 130 }); // centered top logo
//       doc.moveDown(4); // space after logo
//     } catch (e) {
//       console.warn('⚠️ Logo not found at uploads/logo.png:', e.message);
//     }
...
//       ['Customer ID (BSG)', data.bsgCustId ?? ''],
//       ['Sales Rep ID', data.salesRepId ?? ''],
...
// }
function buildApprovalEmailHtml(payload) {
  const bsgRow = payload?.bsgCustId
    ? `<tr><td><b>معرّف العميل (BSG)</b></td><td>${payload.bsgCustId}</td></tr>`
    : '';
  const repRow = payload?.salesRepId
    ? `<tr><td><b>معرّف مندوب المبيعات</b></td><td>${payload.salesRepId}</td></tr>`
    : '';


  return `
    <div dir="rtl" style="font-family:'Cairo',Arial,Helvetica,sans-serif;font-size:14px;color:#111;text-align:right">
      <h2 style="margin:0 0 8px">تمت الموافقة على الاستبدال</h2>
      <p>تمت <b>الموافقة</b> على طلب الاستبدال التالي:</p>
      <table cellpadding="6" cellspacing="0" border="0" style="border-collapse:collapse;width:100%;text-align:right">
        <tr><td><b>الاسم</b></td><td>${payload.name ?? ''} (${payload.role ?? ''})</td></tr>
        <tr><td><b>رقم الجوال</b></td><td>${payload.phone ?? ''}</td></tr>
        <tr><td><b>المكافأة</b></td><td>${payload.rewards ?? ''}</td></tr>
        <tr><td><b>النقاط المطلوبة</b></td><td>${payload.requiredPoints ?? ''}</td></tr>
        ${bsgRow}
        ${repRow}
        <tr><td><b>الموقع</b></td><td>${payload.location ?? ''}</td></tr>
        <tr><td><b>تاريخ الطلب</b></td><td>${prettyDate(payload.requestDate) ?? ''}</td></tr>
        <tr><td><b>تاريخ الموافقة</b></td><td>${prettyDate(new Date())}</td></tr>
      </table>
      <p style="margin-top:16px">تم إرفاق ملخص الاستبدال بصيغة PDF.</p>
      <p style="margin-top:16px">— نظام المكافآت</p>
    </div>
  `;
}

function buildStakeholderEmailHtml(redemption) {
  const bsgRow = redemption?.bsgCustId
    ? `<tr><td><b>معرّف العميل (BSG)</b></td><td>${redemption.bsgCustId}</td></tr>`
    : '';
  const repRow = redemption?.salesRepId
    ? `<tr><td><b>معرّف مندوب المبيعات</b></td><td>${redemption.salesRepId}</td></tr>`
    : '';
  ...
  return `
    <div dir="rtl" style="font-family:'Cairo',Arial,Helvetica,sans-serif;font-size:14px;color:#111;text-align:right">
      ...
      <p>تمت <b>الموافقة</b> على طلب الاستبدال التالي:</p>
      <table cellpadding="6" cellspacing="0" border="0" style="border-collapse:collapse;width:100%;text-align:right">
        <tr><td><b>المستخدم</b></td><td>${redemption?.name ?? ''} (${redemption?.role ?? ''})</td></tr>
        <tr><td><b>رقم الجوال</b></td><td>${redemption?.phone ?? ''}</td></tr>
        <tr><td><b>المكافأة</b></td><td>${redemption?.rewards ?? redemption?.rewardId ?? ''}</td></tr>
           ${bsgRow}
        ${repRow}
        <tr><td><b>النقاط المطلوبة</b></td><td>${redemption?.requiredPoints ?? redemption?.pointsConsumed ?? ''}</td></tr>
        <tr><td><b>الموقع</b></td><td>${redemption?.location ?? ''}</td></tr>
        <tr><td><b>تاريخ الطلب</b></td><td>${prettyDate(redemption?.requestDate)}</td></tr>
        <tr><td><b>تاريخ الموافقة</b></td><td>${prettyDate(new Date())}</td></tr>
      </table>
      <p style="margin-top:16px">تم إرفاق ملخص الاستبدال بصيغة PDF.</p>
      <p style="margin-top:16px">— نظام المكافآت</p>
...
async function sendApprovalEmail(payload) {
  ...
  // Enrich payload with user’s IDs
  if (payload.userId) {
    const user = await User.findByPk(payload.userId).catch(() => null);
    if (user) {
      payload.bsgCustId = user.bsgCustId;
      payload.salesRepId = user.salesRepId;
    }
  }
    ...
    if (redemption?.userId) {
      ...
      if (actor) {
        // Actor email → TO
            ...
            redemption.bsgCustId = actor.bsgCustId || redemption.bsgCustId; 
        redemption.salesRepId = actor.salesRepId || redemption.salesRepId;
        if (actor.email) {
          toRecipients.add(actor.email);
          console.log('[EMAIL] + Added actor email to TO:', actor.email);
        }
        ...
        if (actor.salesRepId) {
          ...
          if (salesRep?.email) {
            bccRecipients.add(salesRep.email);
        ...
        if (salesRep?.branchManagerId) {
          ...
          if (branchManager?.email) {
            bccRecipients.add(branchManager.email);
