...
// 7️⃣ Login SalesRep (also accepts BRANCH_MANAGER when sales_rep_id is stored on that user)
exports.loginSalesRep = async (req, res) => {
  const { salesRepId, password } = req.body;
  const code = String(salesRepId || '').trim();
  if (!code || !password) {
    return res.status(400).json({ success: false, message: 'salesRepId and password are required' });
  }
  console.log(`[LOGIN] Mobile rep login attempt for ID: ${code}`);
  ...
  const user =
    allUsers.find((u) => u.role && u.role.toUpperCase().trim() === 'SALES_REP' && u.salesRepId === code) ||
    allUsers.find((u) => u.role && u.role.toUpperCase().trim() === 'BRANCH_MANAGER');
  if (!user) return res.status(403).json({ success: false, message: 'Invalid credentials' });
