...
function phoneVariants(rawPhone) {
  ...
  const with0 = n.startsWith('0') ? n : '0' + n;
  const with966 = n.startsWith('966') ? n : '966' + n;
...
exports.ensureSuperAdmin = async (req, res) => {
  try {
    ...
    if (existing) return res.status(409).json({ message: 'SUPER_ADMIN already exists', superAdmin: existing });
...
exports.registerUser = async (req, res) => {
  try {
    let { phoneNumber, password, role, fullName, email, binShihonWorkerId, salesRepId, branchManagerId } = req.body;
    ...
    if (!allowed) return res.status(400).json({ message: 'Invalid role for Dashboard platform' });
  ...
  return res.status(403).json({
    ...
    if (user.role === 'SUPER_ADMIN') {
      ...
      return res.status(200).json({
        ...
        token,
        permissions: ['*'],
        user: { id: user.id, phoneNumber: user.phoneNumber, role: user.role, fullName: user.fullName, email: user.email }
      });
    }
 
    // RBAC permissions fetch
    const userWithRoles = await User.findByPk(user.id, {
      include: {
        ...
        include: {
          model: Duty,
          as: 'roleDuties',
          include: { model: Privilege, as: 'dutyPrivileges' }
        }
      }
    });
    ...
    userWithRoles?.userRole?.roleDuties?.forEach(duty => {
      duty.dutyPrivileges?.forEach(priv => permissions.push(priv.controlName));
    });
...
exports.forgotPasswordSendOtp = async (req, res) => {
  try {
    ...
    const allowed = await roleAllowedOnDashboard(user.role);
    if (!allowed) return res.status(403).json({ message: 'This account is not authorized for the Admin Panel' });
 
    const otp = ENABLE_OTP ? Math.floor(1000 + Math.random() * 9000).toString() : '1234';
    ...
    return res.json({ success: true, message: 'OTP sent', expiresAt, ...(ENABLE_OTP ? {} : { otp }) });
  } catch (err) {
...
exports.forgotPasswordVerifyOtp = async (req, res) => {
  try {
    ...
    if (!phoneNumber || !otpCode) return res.status(400).json({ message: 'Phone number and OTP code are required' });
    ...
    return res.json({ success: true, message: 'OTP verified', userId: user.id });
  } catch (err) {
