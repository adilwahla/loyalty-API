...
io.on('connection', (socket) => {
  ...
  if (userId) {
    ...
    socket.join(`user:${userId}`); // Also join user:userId room for task events
  ...
  if (role === 'ADMIN' || role === 'SUPER_ADMIN' || role === 'SALES_ADMIN' || role === 'BRANCH_MANAGER') {
    socket.join('admins');
    socket.join('admin_tasks'); // Join admin tasks room for task updates
  ...
  // ✅ Task-related subscriptions
  socket.on('admin_subscribe_tasks', () => {
    socket.join('admin_tasks');
    console.log(`📋 Admin ${userId || socket.id} subscribed to tasks`);
    socket.emit('admin_tasks_subscribed', true);
  });

  socket.on('user_subscribe_tasks', (data) => {
    const targetUserId = data?.userId || userId;
    if (targetUserId) {
      socket.join(`user:${targetUserId}`);
      console.log(`📋 User ${targetUserId} subscribed to tasks`);
      socket.emit('user_tasks_subscribed', { userId: targetUserId });
    }
  });
